...
import { Camera, MapPin, Users, Loader2, Play, CheckCircle2, Square, Truck } from 'lucide-react';
...
export default function StartRoute() {
  ...
  useEffect(() => {
    if (activeRoute) return; // active route view renders separately — no setup needed
  ...
  useEffect(() => {
    if (location && centers.length > 0) {
      ...
      if (closest) {
        ...
        if (!selectedCenter) setSelectedCenter(closest.name);
      } else {
  ...
  useEffect(() => {
    if (profile?.fixedHelper) setHasHelper(true);
  }, [profile]);
    ...
    try {
      ...
      const routeData = {
        ...
        driverName: profile?.firstName
          ...
          : user.name || user.displayName || 'Conductor',
      ...
      setActiveRoute({ id: docRef.id, ...routeData, startTime: new Date() });
      // activeRoute is now set — the component re-renders to active-route view
    } catch (err) {
      console.error(err);
  ...
  if (activeRoute) {
    return (
      <div className="max-w-md mx-auto">
        <Helmet>
          <title>BTS Logistics Pro - Jornada Activa</title>
        </Helmet>

        <div className="glass p-8 rounded-3xl flex flex-col items-center gap-8">
          <div className="flex flex-col items-center gap-4">
            ...
            <div className="text-center">
              <p className="text-xs text-green-400 uppercase font-bold tracking-widest mb-1">
                Jornada en Curso
              </p>
              <p className="text-xl font-black text-white">{activeRoute.startCenter}</p>
            </div>
          </div>

          <button
            onClick={() => navigate('/driver/end')}
            className="w-full bg-red-600 hover:bg-red-500 text-white font-black py-6 rounded-2xl shadow-lg shadow-red-600/20 flex items-center justify-center gap-3 text-2xl transition-all active:scale-95"
          >
  ...
  return (
    <div className="max-w-md mx-auto space-y-6">
      <Helmet>
        <title>BTS Logistics Pro - Iniciar Ruta</title>
      </Helmet>
      ...
      <div className="glass p-6 rounded-3xl">
        ...
        <form onSubmit={handleStart} className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                {detectedCenter && selectedCenter !== detectedCenter.name && (
                  <p className="text-[10px] text-gray-500 mt-1 italic">
                    Estás cerca de{' '}
                    <span className="text-green-400 font-bold">{detectedCenter.name}</span>
                  </p>
                )}
              </div>
          ...
          >
            ...
            <div
              className={`w-12 h-6 rounded-full p-1 transition-all ${hasHelper ? 'bg-blue-600' : 'bg-gray-700'}`}
            >
              <div
                className={`w-4 h-4 bg-white rounded-full transition-all ${hasHelper ? 'translate-x-6' : ''}`}
              />
            </div>
          </button>
